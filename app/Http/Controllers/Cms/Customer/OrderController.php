...
class OrderController extends Controller
{
    public function index()
    {
        ...
        return view('cms.customer.orders.list', compact(
            'orders',
            'customers',
            'products',
            'currency',
    ...
    public function list($id = null)
    {
        ...
        $ops = $ops->paginate(request("limit"))->through(function ($op) use ($user) {
            ...
            $pdf_route = route("cms.customer.orders.po.pdf", $op->id);
            ...
            if ($op->status->title == 'Processing' || $op->status->title == 'Payment Pending') {
                $actions = $actions . $profile_action . $actions_pdf . $delete_action;
            } else {
                $actions = $actions . $profile_action . $actions_download_pdf;
            }
    ...
    public function getLines($id)
    {
        ...
        $view = view('/cms/customer/orders/_order_details_modal', [
            'order' => $order,
            'lines' => $lines,
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $order_status_id = gerOrderStatusId('Processing');
                ...
                if ($saved) {
                    ...
                    foreach ($request->product_id as $key => $item) {
                        ...
                        $data = new OrderLine([
                            ...
                            'chicken_quantity' => $request->chicken_quantity[$key],
                            'meat_quantity' => $request->meat_quantity[$key],
                            'vegetarian_quantity' => $request->vegetarian_quantity[$key],
    ...
    public function downloadQrPdf()
    {
        ...
        $pdf = Pdf::loadView('cms.customer.vouchers.vpdf', compact('codes'))->setPaper('a4');
    ...
    public function switch($id)
    {
        if ($id) {
            if (Event::findOrFail($id)) {
                ...
                return redirect()->route('cms.customer.orders')->with('message', 'Event Switched.');
    ...
    public function pickEvent(Request $request)
    {
        ...
        return redirect()->route('cms.customer.orders')->with('error', 'Event not found.');
        // }
    }
}
